...
const dragTaskEpic = action$ =>
  ...
  // .do(({destination: {index}}) => index < 0 && snackbarMessage({ message: 'Destination must be in task list zone' }))
  .filter(({
    destination: {
      index
    }
  ...
  .map(
    ({
      ...
      desIndexInDb,
      desSiblingIndexInDb: destinationIndex + 1 === R.length(allTasks) ?
        desIndexInDb - 1 :
        !destinationIndex ?
        desIndexInDb + 1 : R.prop(
          'indexInDb',
          R.nth(
